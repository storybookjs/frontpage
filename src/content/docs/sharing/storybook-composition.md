...
💡 `storybook@latest extract` uses [Puppeteer](https://www.npmjs.com/package/puppeteer), which downloads and installs Chromium. Set the environment `SB_CHROMIUM_PATH` to configure your local Chromium installation.
...
```shell
npx storybook@latest extract my-built-storybook-directory my-other-directory/index.json
```
